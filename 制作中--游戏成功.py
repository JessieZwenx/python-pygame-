...
def draw_button(rect,text,color,hover_color,font=button_font):
    ...
    # 按钮表面的文字
    text_surf = font.render(text,True,(0, 40, 80) )
    # 按钮文字的where
...
def draw_game_over_screen():
    if game_over_reason == "游戏失败":
        screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/death.jpg"),(screen_width,screen_height)),(0,0))
        # screen.fill(BLACK)
        ...
        return restart_button,game_over_text,game_over_reason
    
    if game_over_reason == "游戏成功":
        screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/win.png"),(screen_width,screen_height)),(0,0))
        # screen.fill(BLACK)

        # 胜利原因
        game_success_text = title_font.render("游戏成功",True,WHITE)
        screen.blit(game_success_text,(100,300))

        game_success_reason =normal_font.render ("恭喜你，你成功在太空中生活了十天。",True,WHITE)
        screen.blit(game_success_reason,(100,400))
        ...
        return restart_button,game_success_text,game_success_reason
    


    

def draw_game_success_screen():
    screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/win.jpg"),(screen_width,screen_height)),(0,0))
    # screen.fill(BLACK)
...
def draw_start_screen():
    screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/1.png"),(screen_width,screen_height)),(0,0))

    
    pygame.draw.rect(screen,(WHITE),(210,100,850,250))
    screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/3.png"),(750,300)),(250,90))
    ...
    screen.blit(subtitle_surface,(screen_width // 2 - subtitle_surface.get_width() // 2,250))
    ...
    for i,desc in enumerate(descriptions):
        ...
        # 悬停按钮变化
        buttonstart_color = BUTTON_HOVER  if is_hover else YELLOW
        # 按钮的内部填充
        ...
        # 按钮的文字
        start_text = button_font.render("开始游戏",True,(0, 20, 40) )
...
def draw_main_interface():
    # 背景
    screen.blit(pygame.transform.scale(pygame.image.load("../python-game-spacecontrol/sources/photos/2.jpg"),(screen_width,screen_height)),(0,0))
    ...
    # screen.blit(panel_title,(100,160))
...
# 中间括号是长宽高
    ...
    panel_title = header_font.render("模块状态",True,(0, 40, 80))
...
# 动态列表，遍历所有模块。如果有info.append（）或者有info.pop（）就是长度或内容随时变化啊 的动态列表
    ...
    panel_title = header_font.render("船员状态",True,(0, 40, 80))
...
def end_day():
    ...
    production = {
        ...
        "食物": min(4 + totle_crew,resources["食物"]),
        "水源": min(2 + totle_crew,resources["水源"]),
        "氧气": min(1 + totle_crew,resources["氧气"]),
        "材料": min(4 + totle_crew,resources["材料"])
    }
# 对于资源创造
    for resource,anmount in production.items():
        resources[resource] += anmount
    # 从资源中减去消耗
    ...
    if resources["能源"] >=30:
        ...
        if"材料"in message:
            ...
            for resource in ["能源", "食物", "水源", "氧气", "材料"]:
                resources[resource] += random.randint(1,4)
...
def check_game_over():
    # print("检查游戏是否结束...")
    global game_over,game_success,game_over_reason
    creitical_resources = ["能源","食物","水源","氧气"]
        
    if any (value <=0 for value in[resources["能源"],resources["水源"],resources["氧气"
    ],resources["食物"]] ):
        print("游戏失败！")
        game_over_reason = "游戏失败"
        game_over = True
        game_success = False
        return game_success,game_over_reason,game_over
    elif day >= 10:
        print("游戏成功！")
        game_over_reason = "游戏成功"
        game_success = True
        game_over = False
        return game_success,game_over_reason,game_over
...
def main():
    ...
    # 游戏正式开始了
    # 对全局变量的管理
    global game_started,day,resources,message,game_over,game_success,game_over_reason
    ...
    while running:  
        ...
        # 游戏中检查是不是拥有游戏结束的条件
        if game_started and not game_over and not game_success:
            if check_game_over():
                if game_over_reason == "游戏成功":
                    game_success = True
                    print("game_success")
                elif game_over_reason == "游戏失败":
                    game_over = True
                    print("game_over")
        ...
        # 游戏开始和主界面
        if not game_started and not game_over and not game_success:
            # 开始界面
            start_buttons_rect = draw_start_screen()
            # 按钮释放内存，防止悬空引用带来的报错
            buttons = None
            back_button_rect = None
        elif  game_started and not game_over and not game_success:
            # 主界面
            ...
            if event.type == pygame.QUIT:
                ...
                if not game_started:
                    ...
                    if start_button.collidepoint(event.pos):
                        game_started = True
                elif game_over and game_over_reason == "游戏失败":
                    if restart_button and restart_button.collidepoint(event.pos):
                        reset_game()
                        game_started = False
                        game_over = False
                        game_success = False
                elif game_over and game_over_reason == "游戏成功":
                    if restart_button and restart_button.collidepoint(event.pos):
                        reset_game()
                        game_started = False
                        game_over = False
