...
def end_day():
    ...
    consumption = {
        "能源": max(0,10 + totle_crew),
        "食物": max(0,8 + totle_crew),
        "水源": max(0,6 + totle_crew),
        "氧气": max(0,5 + totle_crew)
    }
    # 对于资源创造
    production = {
        "能源": min(10 + totle_crew,resources["能源"]),
        "食物": min(8 + totle_crew,resources["食物"]),
        "水源": min(6 + totle_crew,resources["水源"]),
        "氧气": min(5 + totle_crew,resources["氧气"]),
        "材料": min(5 + totle_crew,resources["材料"])
    }
# 对于资源创造
...
def explore():
    ...
    resources["食物"] -= max(0,random.randint(0,5))
...
def main():
    ...
    while running:  
        ...
        if not game_started and not game_over:
            ...
            # 处理事件
        #    显示结束界面
            restart_button,game_over_text,game_over_reason = draw_game_over_screen()
