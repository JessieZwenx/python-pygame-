...
def draw_start_screen():
    ...
    for i,desc in enumerate(descriptions):
        ...
        # 开始按钮
    button_start_width,button_start_heigh = 200,60
    button_start_x = screen_width // 2 - button_start_width // 2
    button_start_y = 550

       

# 这个在外面就不会导致开始按钮只执行一次了
        # 检测鼠标是否在按钮上
    mouse_pos = pygame.mouse.get_pos()
    button_start_rect = pygame.Rect(button_start_x,button_start_y,button_start_width,button_start_heigh)
        # collidepoint检测碰撞是否在矩形内
    is_hover = button_start_rect.collidepoint(mouse_pos)

        

        # 悬停按钮变化
    button_start_color = BUTTON_HOVER  if is_hover else BUTTON_GREEN
        # 按钮的内部填充
    pygame.draw.rect(screen,button_start_color,button_start_rect,border_radius=12)
        # 按钮的边框样式，区别在于中间有参数2是边框粗细
    pygame.draw.rect(screen,WHITE,button_start_rect,3,border_radius=12)

        # 按钮的文字
    start_text = button_font.render("开始游戏",True,WHITE)
        # 让start_text获取按钮矩形范围，然后按钮的中心作为中心，居中，达成文字整体居中
    text_rect = start_text.get_rect(center=button_start_rect.center)
    ...
    #  存储按钮的位置==将按钮的区域存储到字典中
    button_positions["button_start_rect"] = button_start_rect

    return button_start_rect
...
def draw_game_over_screen():
    ...
    # 绘制游戏结束标题
    title_surface = title_font.render("游戏结束",True,WHITE)
    screen.blit(title_surface,(screen_width //2 - title_surface.get_width() // 2,200))

    # 绘制失败原因
    reason_text = normal_font.render(f"由于{game_over_reason}耗尽，无人生还...",True,YELLOW)
    screen.blit(reason_text,(screen_width //2 - reason_text.get_width() // 2,300))

    # 绘制统计信息
    stats_text = normal_font.render(f"此次生存了{day}天",True,WHITE)
    screen.blit(stats_text,(screen_width //2 - stats_text.get_width() // 2,340))

    # 重新开始按钮
    button_restart_width,button_restart_heigh = 200,60
    button_restart_x = screen_width // 2 - button_restart_width // 2
    button_restart_y = 450
    button_restart_rect = pygame.Rect(button_restart_x,button_restart_y,button_restart_width,button_restart_heigh)

    draw_button(button_restart_rect,"重新开始",BUTTON_GREEN,BUTTON_HOVER)

    return button_restart_rect
...
def check_game_over():
    ...
    for resource in creitical_resources:
        if resources[resource]<=0:
            game_over = True
            game_over_reason = f"{resource}耗尽"
            # 游戏结束条件满足，返回true，出现游戏结束画面
            return True
    # 条件不满足，返回false，游戏继续进行
    return False
...
def draw_main_interface():
    ...
    # 绘制天数
    day_surface = title_font.render(f"第 {day} 天",True,WHITE)
...
# 动态列表，遍历所有模块。如果有info.append（）或者有info.pop（）就是长度或内容随时变化啊 的动态列表
    ...
    button_positions["main_buttons"] = buttons
    button_positions["back_button"] = back_button_rect
...
def main():
    ...
    while running:  
        ...
        if game_started and  game_over==False:
            check_game_over()
        ...
        if not game_started:
            # 开始界面
            button_start_rect = draw_start_screen()
            # 按钮释放内存，防止悬空引用带来的报错
            buttons = None
            ...
            # 主界面
            buttons,button_restart_rect = draw_main_interface()
        ...
        for event in pygame.event.get():
            ...
            # 游戏结束界面的按钮
                ...
                #         resources[resource] = max(0,resources[resource] + change)
                    ...
                    if buttons and button_restart_rect:
                        ...
                        elif button_restart_rect.collidepoint(event.pos):
                            game_started = False
